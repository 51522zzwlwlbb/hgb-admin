<template>
  <n-form v-bind="getBindValue" :model="formModel" ref="formElRef">
    <n-grid v-bind="getGrid">
        ...
        <n-form-item :label="schema.label" :path="schema.field">
          ...
          <template #label v-if="schema.labelMessage">
            ...
            <n-tooltip trigger="hover" :style="schema.labelMessageStyle">
              <template #trigger>
                <n-icon size="18" class="text-gray-400 cursor-pointer align-sub">
          ...
          <template v-else-if="schema.component === 'NCheckbox'">
            <n-checkbox-group v-model:value="formModel[schema.field]">
              <n-space>
                <n-checkbox v-for="item in (schema.componentProps as any).options" :key="item.value" :value="item.value" :label="item.label" />
              </n-space>
            </n-checkbox-group>
          </template>
          ...
          <template v-else-if="schema.component === 'NRadioGroup'">
            <n-radio-group v-model:value="formModel[schema.field]">
              <n-space>
                <n-radio v-for="item in (schema.componentProps as any).options" :key="item.value" :value="item.value">
                  {{ item.label }}
                </n-radio>
              </n-space>
            </n-radio-group>
          ...
          <!--动态渲染表单组件-->
          <template v-else-if="schema.component">
            <component
              :is="schema.component"
      ...
      <!--提交 重置 展开 收起 按钮-->
      <n-gi #="{ overflow }" v-if="getProps.showActionButtonGroup" :suffix="true" class="flex items-baseline justify-end h-full">
        <n-space align="center" :justify="isInline ? 'end' : 'start'" :style="{ 'margin-left': `${isInline ? 12 : getProps.labelWidth}px` }">
          <n-button v-if="getProps.showSubmitButton" v-bind="getSubmitBtnOptions" @click="handleSubmit" :loading="loadingSub">{{
