  ...
  <div class="tableAction">
    <div class="flex items-center justify-center gap-1">
      <template v-for="(action, index) in getActions" :key="`${index}-${action.label}`">
        <n-button v-bind="action" class="mx-2" :data-index="index">
          {{ action.label }}
      ...
      <n-dropdown v-if="dropDownActions && getDropdownList.length" trigger="hover" :options="getDropdownList" @select="handleSelect">
        <slot name="more"></slot>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      // 触发配置项的select方法并将key 和 点击选中的配置项返回
      const handleSelect = (key: string, actions: any) => {
        props.select(key, actions);
      };
